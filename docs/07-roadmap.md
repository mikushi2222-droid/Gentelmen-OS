...
### V2.12 — Релизная подготовка

**Приоритет: Зависит от готовности V2.11.**

#### A. Визуальная идентичность
- Иконка приложения 512×512 + адаптивная для Android 13+
- Splash-screen (`flutter_native_splash`)
- `applicationId = com.gentleman.os`, версия `1.0.0+1`

#### B. Релизный билд
- Keystore + `key.properties` (в `.gitignore`, не в репо)
- ProGuard/R8 правила для Drift + sqlite3
- `flutter build appbundle --release` в CI на тег `v1.0.0`

#### C. Play Store
- Описание RU/EN, скриншоты, feature graphic 1024×500
- Privacy Policy (offline, данные не покидают устройство)
- Data Safety форма: «Нет сбора, нет передачи»
- Категория: Lifestyle / Health & Fitness
- Внутренний трек → Closed Testing → Production

---

## 3. Бэклог v1.1+ (после релиза)
...
| B5 | iOS-сборка | Расширение аудитории |
| B6 | Тёмная + светлая тема (переключатель) | UX |
| B7 | Мультиязычность (en) | Выход за RU-аудиторию |
| B8 | Онлайн AI без RouterAI (OpenAI/Gemini) | Альтернативный провайдер |
| B9 | Wear OS mini-watch face | Gentleman Score на запястье |
...
5. **Один источник правды** по командам сборки — README.
6. **Нет преждевременных абстракций** — три похожих строки лучше абстракции.
...
| Риск | Митигация |
|------|-----------|
| Раздувание scope | Жёсткие анти-цели из PRD; V2.9–V2.12 ограничены |
| Снижение мотивации | RPG-слой + чёткие milestone с DoD |
| Регрессии при полировке | Widget-тесты + code-review после каждой итерации |
| Сложность релиза | Фаза 5 начинается только после зелёного CI |
