...
- **`main` — основная (дефолтная) и единственная релизная ветка.** Любая фича
  **обязательно** доводится до `main`: коротка feature-ветка от свежего `main` →
  PR в `main` → мёрж → удалить ветку. Ветка, не влитая в `main`, считается
  незавершённой; не накапливай параллельные долгоживущие истории (это уже
  приводило к расхождению схемы БД — см. `docs/18-branch-consolidation-plan.md`).
- Разработка — на выделенной ветке, не прямо в `main`.
- **Промоушен в `main` после зелёного CI.** Как только GitHub Actions-прогон
  (workflow `CI`: analyze + tests + build) для feature-ветки завершился
  **успешно** — слей ветку в `main` и запушь `main`. Это и есть «доведение до
  main»; делай это **всегда** после зелёного прогона, не дожидаясь отдельной
  просьбы. Не пушь в `main` при **красном/отсутствующем** CI и не делай
  force-push. Если CI не может выполниться (нет раннера/биллинга — см.
  `docs/15-ci-and-build.md`) — скажи об этом и жди, а не промоуть непроверенное.
