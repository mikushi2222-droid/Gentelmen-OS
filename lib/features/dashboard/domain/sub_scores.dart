...
/// Если даже слабейшее звено уже сильное (≥80), не выдумываем проблему, а
/// поддерживаем темп — честность важнее «движка ради движка» (CLAUDE.md §1.4).
String dailyTip(SubScores scores) {
  final weakest = scores.weakest;
  if (weakest.value >= 80) {
    return 'Все направления в хорошей форме. Удерживайте темп — '
        'постоянство решает больше, чем рывки.';
  }
  return switch (weakest.name) {
